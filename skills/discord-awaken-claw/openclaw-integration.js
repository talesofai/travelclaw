...
// ─── Config ───────────────────────────────────────────────────────────
const STATE_FILE = path.join(__dirname, 'reference/state.json');
const SOUL_FILE = path.join(__dirname, '../../SOUL.md');
const ORIGINAL_SOUL_FILE = path.join(__dirname, 'reference/SOUL.md.original');
...
function isButtonInteraction(customId) {
  ...
  const prefixes = ['start_', 'answer_', 'manual_', 'confirm_yes_', 'confirm_no_', 'travel_'];
...
async function handleDiscordMessage(context, callLLM) {
  ...
  try {
    if (interactionType === 'button' && customId) {
      ...
      // Auto-triggered channel join button
      if (customId.startsWith('start_channel_')) {
        const game = newGame(channelId, guildId);
        game.started = true;
        game.waitingFor = 'word';
        setGame(userId, game);
        await promptInitialWord(channelId, sendMessage);
